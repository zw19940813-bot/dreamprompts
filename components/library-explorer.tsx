    ...
    <div>
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search by task, tool, or idea..."
          className="focus-ring h-14 w-full rounded-xl border border-black/10 bg-white pl-14 pr-5 text-base shadow-sm placeholder:text-muted"
          aria-label="Search prompt library"
        />
      </div>

      <div className="mt-5 flex flex-wrap gap-2" aria-label="Filter prompts by tool">
        {filters.map((item) => (
          <button
            key={item}
            onClick={() => setTag(item)}
            className={`focus-ring rounded-full px-4 py-2 text-sm font-semibold transition ${
              tag === item ? 'bg-blue text-white' : 'bg-soft text-muted hover:text-ink'
            }`}
      ...
      <p className="mt-8 text-sm text-muted">
        Showing {results.length} {results.length === 1 ? 'prompt' : 'prompts'}
      </p>
      ...
      {!results.length && (
        <div className="my-20 text-center">
          <p className="serif text-2xl font-bold">No prompts found.</p>
          <p className="mt-2 text-sm text-muted">Try a broader search or another tool.</p>
        </div>
      )}
    </div>
  )
